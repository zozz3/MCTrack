...
class Kitti2DBox(_BaseDataset):
    ...
    @staticmethod
    def get_default_dataset_config(cfg):
        """Default class config values"""
        code_path = utils.get_code_path()
        # res_path = "results/KITTI/Car"
        default_config = {
            'GT_FOLDER': os.path.join(code_path, '../gt'),  # Location of GT data
            'TRACKERS_FOLDER': f'{cfg["SAVE_PATH"]}/{cfg["DETECTOR"]}',  # Trackers location
            # 'TRACKERS_FOLDER': 'results/kitti',  # Trackers location
            # 'TRACKERS_FOLDER': res_path,  # Trackers location
            'OUTPUT_FOLDER': None,  # Where to save eval results (if None, same as TRACKERS_FOLDER)
            'TRACKERS_TO_EVAL': None,  # Filenames of trackers to eval (if None, all in folder)
            'CLASSES_TO_EVAL': [item.lower() for item in cfg["CATEGORY_LIST"]], # ['car', 'pedestrian'],   Valid: ['car', 'pedestrian']
    ...
    def __init__(self, config=None, cfg=None):
        ...
        self.config = utils.init_config(config, self.get_default_dataset_config(cfg), self.get_name())
        ...
        self.seq_list = [str(seq).zfill(4) for seq in cfg["TRACKING_SEQS"]]
        self.seq_lengths = {key: self.seq_lengths[key] for key in self.seq_list}
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        if self.data_is_zipped:
            ...
            if is_gt:
                ...
                file = os.path.join(self.tracker_fol, self.config['SPLIT_TO_EVAL'], self.tracker_sub_fol, seq + '.txt')
        ...
        for t in range(num_timesteps):
            ...
            if time_key in read_data.keys():
                time_data = np.float64(np.asarray(read_data[time_key]))
            ...
            if is_gt:
                if time_key in ignore_data.keys():
                    time_ignore = np.float64(np.asarray(ignore_data[time_key]))
