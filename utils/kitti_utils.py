...
def save_scene_results(scene_id, scene_result, cfg):
    eval_split = "training" if cfg["SPLIT"] == "val" else cfg["SPLIT"]
    save_path = os.path.join(cfg["SAVE_PATH"], cfg["DETECTOR"], eval_split, "data")
    os.makedirs(save_path, exist_ok=True)

    save_name = os.path.join(save_path, scene_id + ".txt")

    save_image_dir = os.path.join(
        cfg["SAVE_PATH"], cfg["DETECTOR"], eval_split, "image", scene_id
    )
